...
export function HouseStatsPanel({ houses, selectedFilter, onFilterChange, allHouses }: HouseStatsPanelProps) {
  // Calculate global statistics from all houses for navigation purposes
  const total = allHouses.length
  const visited = allHouses.filter((house) => house.isVisited && !house.isNotAvailable).length
